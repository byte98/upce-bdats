...
public class XMLParser
{
    ...
    /**
     * Parses key used to store data from XML node
     * @param node XML node containing key
     * @return Type of key used to store data from XML node,
     *         or NULL if key cannot be parsed
     */
    public eTypKey parseKey(Node node)
    {
        eTypKey reti = null;
        if (node.getNodeName().equals(XMLFormat.XML_KEY))
        {
            Node val = node.getFirstChild();
            if (val.getNodeValue().equals(XMLFormat.XML_KEY_GPS))
            {
                reti = eTypKey.GPS;
            }
            else if (val.getNodeValue().equals(XMLFormat.XML_KEY_NAME))
            {
                reti = eTypKey.NAME;
            }
        }
